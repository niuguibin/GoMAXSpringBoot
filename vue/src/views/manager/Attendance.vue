  ...
  <div>
    <div class="search">
      <el-select v-model="courseId" placeholder="请选择课程" style="width: 200px">
        <el-option v-for="item in courseSearchData" :label="item.name"   :key="item.id"   :value="item.id"></el-option>
      </el-select>
      ...
      <el-form label-width="100px" style="padding-right: 50px" :model="form" :rules="rules" ref="formRef">
        <el-form-item prop="courseId" label="选择课程">
          <el-select v-model="form.courseId" placeholder="请选择课程" style="width: 100%" @change="getStudent">
            <el-option v-for="item in courseData" :label="item.name"  :key="item.id"     :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="studentId" label="选择学生">
          <el-select v-model="studentId" placeholder="请选择学生" style="width: 100%">
            <el-option v-for="item in studentData" :label="item.studentName"  :key="item.studentId" :value="item.studentId"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
      ...
      if ('STUDENT' === this.user.role) {
        this.$request.get('/choice/selectAll?studentId=' + this.user.id).then(res => {
          if (res.code === '200') {
            ...
            this.courseSearchData = res.data//如果是学生 请求choice接口的数据返给courseSearchData
          }
        })
      } else {
        ...
        this.$request.get(url).then(res => {
          if (res.code === '200') {
            this.courseSearchData = res.data  ////如果是管理员  请求course接口的数据返给courseSearchData
          } else {
    ...
    getStudentEdit(courseId) {//点击编辑的时候，将这行数据获取到
      this.$request.get('/choice/selectAll', {
        params: {
          courseId: courseId
        }
