      ...
      <el-form :model="form" label-width="100px" style="padding-right: 50px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="学院" prop="collegeId">
          <el-select v-model="form.collegeId" placeholder="请选择学院" style="width: 100%">
            <el-option v-for="item in collegeData" :label="item.name"  :key="item.id"    :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="专业" prop="specialityId">
          <el-select v-model="form.specialityId" placeholder="请选择专业" style="width: 100%">
            <el-option v-for="item in specialityData" :label="item.name"  :key="item.id"    :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="班级" prop="classId">
          <el-select v-model="form.classId" placeholder="请选择班级" style="width: 100%">
            <el-option v-for="item in classDate" :label="item.name"  :key="item.id"    :value="item.id"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleAvatarSuccess(response, file, fileList) {//上传成功后，会调用钩子函数，钩子函数会有个response参数会接收 后端上传成功时给的一个返回值，即是图片的名称
      //即FileController下的   return Result.success(http + flag + "-" + fileName);
